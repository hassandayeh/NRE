...
function ParticipantsPanel(props: {
  ...
  const { bookingId, orgId, onParticipantsRefetched } = props;

  // helper to append orgId (when present) to any URL
  ...
  async function refetch() {
    try {
      ...
      const res = await fetch(
        withOrg(`/api/bookings/${bookingId}/participants`),
        {
          credentials: "include",
          cache: "no-store",
        }
  ...
  async function onAdd(e: React.FormEvent) {
    ...
    try {
      setAdding(true);
      await fetch(withOrg(`/api/bookings/${bookingId}/participants`), {
        method: "POST",
        credentials: "include",
  ...
  async function updateParticipantInviteStatus(
    ...
    const attempts = [
      {
        method: "POST",
        url: withOrg(`/api/bookings/${bookingId}/participants`),
        body: JSON.stringify({ participants: [{ id, inviteStatus }] }),
      },
      {
        method: "POST",
        url: withOrg(
          `/api/bookings/${bookingId}/participants?id=${encodeURIComponent(id)}`
        ),
  ...
  return (
    <div className="rounded-xl border bg-white">
      <div className="border-b px-4 py-3 text-sm font-medium">Participants</div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            {!!invite && (
                              <span
                                className={`rounded px-1.5 py-0.5 text-xs ${pill}`}
                              >
                                {invite}
                              </span>
                            )}
                            ...
                            <select
                              value={invite}
                              onChange={(e) =>
                                onUpdateStatus(p, e.target.value)
                              }
                              disabled={saving}
    ...
    <div className="rounded-xl border bg-white p-3">
      ...
      {open && (
        <div className="mt-3 grid gap-3">
          ...
          {!loading && !error && items.length === 0 && <div>No matches.</div>}
                    ...
                    <div className="flex items-center justify-between">
                      <div className="min-w-0">
                        ...
                        <div className="text-xs">
                          <span
                            className={`mr-2 rounded px-1.5 py-0.5 ${roleBadge}`}
                          >
                            {p.kind}
                          </span>
  ...
  function hostStatus(it: any): "AVAILABLE" | "BUSY" | "UNKNOWN" {
      ...
      if (Array.isArray((a as any).busy) && windowStart && windowEnd) {
        const hit = (a as any).busy.some((w: any) =>
          overlaps(
            windowStart!,
            windowEnd!,
            (w as any)?.start ?? (w as any)?.s,
            (w as any)?.end ?? (w as any)?.e
          )
        );
    ...
    if (arrays.length && windowStart && windowEnd) {
      ...
      for (const arr of arrays) {
        ...
        sawAny = true;
        for (const w of arr) {
          const s = (w as any)?.start ?? (w as any)?.s ?? (w as any)?.[0];
          const e = (w as any)?.end ?? (w as any)?.e ?? (w as any)?.[1];
      ...
      {open && (
        <div className="mt-3 grid gap-3">
          ...
          {!loading && !error && items.length === 0 && (
            <div>No host directory available.</div>
          )}
              ...
              return (
                <li key={h.id}>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="min-w-0">
                        <div className="truncate">{h.name || "Unnamed"}</div>
                        <div
...
export default function BookingEditPage() {
  ...
  const sessionOrgId = sessionReady
    ...
    : null;
  // Client side we pass override if present; server still validates.
  ...
  // helper to append orgId to any API URL
  const withOrg = React.useCallback(
    (url: string) =>
      effectiveOrgId
        ? `${url}${url.includes("?") ? "&" : "?"}orgId=${encodeURIComponent(
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const rb = await fetch(withOrg(`/api/bookings/${id}`), {
          credentials: "include",
        });
        ...
        if (MULTI_PARTICIPANTS_ENABLED) {
          const rp = await fetch(withOrg(`/api/bookings/${id}/participants`), {
            credentials: "include",
            cache: "no-store",
          });
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [id, sessionReady, effectiveOrgId]);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      // Save booking core — NOW including orgId
      const res = await fetch(withOrg(`/api/bookings/${id}`), {
        method: "PATCH",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
      ...
      // brief success toast, then navigate back to VIEW with banner trigger
      setOk("Saved.");
      setTimeout(() => {
        const suffix = overrideOrgId
          ? `?orgId=${encodeURIComponent(overrideOrgId)}&updated=1`
          : `?updated=1`;
        router.push(`/modules/booking/${id}${suffix}`);
      }, 300);
    } catch (e) {
  ...
  return (
    <>
      <h1 className="mb-4 text-2xl font-semibold">Edit booking</h1>
      ...
      <form onSubmit={onSubmit} className="grid gap-4">
            ...
            <>
              {guestsSharedProvisioned ? (
                ...
                <p className="text-sm text-gray-600">
                  Each guest selects their own appearance and access.
                  Provisioning is per guest.
                </p>
              )}
            </>
          ) : null}
                ...
                <>
                  ...
                  <label className="grid gap-1">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        patch({ locationAddress: e.target.value })
                      }
                      className={clsx(
                        "w-full rounded-md border px-3 py-2",
        ...
        <div>
          ...
          {(form.guests || []).length === 0 && (
            <div className="rounded-md border bg-gray-50 p-2 text-sm text-gray-700">
              No guests yet. <br />
              Use “Add guest” below to append experts or reporters.
            </div>
          )}
        ...
        <div className="grid gap-3">
          ...
          <div>
            ...
            {(form.hosts || []).map((h, idx) => {
              ...
              const unifiedType = hostUnifiedType;
                ...
                >
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    <div className="flex gap-1">
                      <UIButton type="button" onClick={() => moveHost(idx, -1)}>
                        ↑
                      </UIButton>
                      <UIButton type="button" onClick={() => moveHost(idx, +1)}>
                        ↓
        ...
        <div className="mt-4 flex items-center gap-2">
          <UIButton
            type="button"
            onClick={() => router.push(`/modules/booking/${id}`)}
          >
            Cancel
          </UIButton>
          <UIButton
